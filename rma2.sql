...
CREATE SCHEMA IF NOT EXISTS `rma` ;
...
CREATE  TABLE IF NOT EXISTS `rma`.`persona` (
  ...
  CONSTRAINT `fk_id_doc_ident`
    ...
    REFERENCES `mydb`.`documento_identificacion` (`id_doc_ident` )
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB
...
ENGINE = InnoDB
COMMENT = 'Relacion productos adquiridos,la cantidad y precio d la compra';
